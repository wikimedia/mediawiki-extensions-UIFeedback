...
( function( mw, $ ) {
	...
	function browserSupported() {
		...
		var versionArray_int = new Array();
		...
		if( name === 'firefox' && versionArray_int[0] > 3 ) return true;
		if( name === 'firefox' && versionArray_int[0] == 3 && versionArray_int[1] >= 6 ) return true;
		if( name === 'opera' && versionArray_int[0] >= 12 && versionArray_int[1] >= 12 ) return true;
...
//			   window.location = mw.util.wikiGetlink( 'Special:UiFeedback' );
	...
	function set_uifeedback_filter() {
		...
		var filter_importance = new Array();
		...
		if( !browserSupported() ) {
			...
			try {
				$( wb ).on( 'stopItemPageEditMode', function( a, origin ) {
					...
					var color = 'green';
					if( use_html2canvas ) color = 'purple';
