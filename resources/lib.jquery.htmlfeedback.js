...
(function( $ ) {
	...
	HTMLFeedback.defaults = {
		 ...
		 */
		onRectangleEnd: function( rectangle, x, y ) { /* rectangle.text("Click me to remove ");*/

			rectangle.mouseover( function( e ) {
				//rectangle.text("Click me to remove ");
				$( rectangle ).find( 'div' ).show();

			} );

			rectangle.mouseout( function( e ) {
				//rectangle.text("");
				$( rectangle ).find( 'div' ).hide();
			} );
			/**/
		},
	...
	HTMLFeedback.init = function( element, options ) {
		...
		var markers = $( "<div />" ).css( css.base ).css( css.markers ).css( css.unselectable ).appendTo( element ).addClass( 'markers' );
		...
		markers.mousedown( function( e ) {
			...
			rectangle = $( "<div />" ).css( {
												"left": e.pageX,
												"top": e.pageY
											} ).css( $.extend( css.rectangle, css.unselectable, {
			...
			rectangle.addClass( 'htmlfeedback-rect' );
